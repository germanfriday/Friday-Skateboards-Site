...
function add_wp_cycle_menu() {
    ...
    add_menu_page('Featured Images Manager', 'Featured Images', 'edit_themes', 'wp-cycle', 'wp_cycle_admin_page', get_bloginfo('template_directory') . '/images/ico_galleries.png', 9);
...
function wp_cycle_handle_upload() {
	...
	if($width > 599 || $height > 300) {
		//	resize the image
		$resized = image_resize($file, 599, 300, true, 'resized');
	...
	//	make the thumbnail
	$thumb_height = round((100 * 300) / 599);
...
function wp_cycle_images_admin() { ?>
	...
	<table class="form-table">
		<tr valign="top"><th scope="row">Upload New Image<br/><small>(must be larger than 599x300, will be scaled to fit.)</small></th>
			<td>
...
function wp_cycle($args = array(), $content = null) {
	...
	if( count($wp_cycle_images) > 0 ):
		...
		foreach((array)$wp_cycle_images as $image => $data) {
			...
			echo '<img src="'.$data['file_url'].'" width="599" height="300" class="'.$data['id'].'" alt="" />';
		...
		echo "<p>" . __('Please go to the "Featured Posts" section in your WordPress admin to add new homepage images.') . "</p>".$newline;
		echo '</div>'.$newline;
...
function wp_cycle_scripts() {
	if(!is_admin())
	wp_enqueue_script( 'jquery.cycle', get_bloginfo('template_url') . '/library/scripts/jquery.cycle.all.min.js', array('jquery','jquery.easing'), '', true );
...
function audio_slider(){
	...
	global $up_options; ?>
            ...
            <div id="featured"<?php if($up_options->shadow == true) echo ' class="shadow"' ?>>
                
                <?php wp_cycle(); ?>

                <div id="album">
	
				<?php wp_reset_query(); query_posts('post_type=album&posts_per_page=1'); ?>
				...
				$master_link = get_permalink(get_the_ID());
				
				if( $itunes_link )
					$master_link = $itunes_link;
				
				if( $amazon_link )
					$master_link = $amazon_link;
				
				if( $custom_link )
					$master_link = $custom_link;
				...
				?>
                	...
                	<div id="buy">
                		...
                		<div class="artwork_container">
							...
							<?php if( $master_link ): ?>
                                <a href="<?php echo $master_link; ?>">
                                    <div class="overlay"></div>
                                    <?php the_post_thumbnail(array(103,103)); ?>
                                </a>
	            			<?php else: ?>
							    <div class="overlay"></div>
								<?php the_post_thumbnail(array(103,103)); ?>
            			...
            			<div class="buy_links">
	            			...
	            			if( $master_link ): ?>
	            			<p class="linkage"><a href="<?php echo $master_link; ?>"><?php 
							if( $up_options->custom_buy_text )
								echo $up_options->custom_buy_text;
							else
								_e('Buy our <span>new</span> album',"garageband"); 
							?></a></p>
	            			<?php endif; ?>

	            			<ul>
	            				<?php if($itunes_link): ?><li class="itunes"><a href="<?php echo $itunes_link; ?>"><?php _e('Buy from iTunes','garageband'); ?></a></li><?php endif; ?>
	            				<?php if($amazon_link): ?><li class="amazon"><a href="<?php echo $amazon_link; ?>"><?php _e('Buy from Amazon','garageband'); ?></a></li><?php endif; ?>
				...
				<?php else: ?>
					...
					<h2 class="no-albums"><?php _e("No Albums Uploaded"); ?></h2>
					<p class="no-albums"><?php _e("Please add at least one album to see something here. This area will show the first three tracks from your latest album."); ?></p>
...
add_action('above_index_content','audio_slider');