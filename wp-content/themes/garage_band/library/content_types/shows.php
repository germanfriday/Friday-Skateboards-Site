...
function shows() {
	...
	$args = array(
		...
		'menu_icon' => get_bloginfo('template_url') . '/images/ico_shows.png',
		'menu_position' => 5,
    	'hierarchical' => false,
    	'rewrite' => true,
...
function admin_init(){
    ...
    wp_enqueue_script( 'anytime', get_bloginfo('template_url') . '/library/scripts/anytimec.js', array('jquery','jquery-ui-core') );
    wp_enqueue_style( 'anytime', get_bloginfo('template_url') . '/library/styles/anytime.css' );
    wp_enqueue_style( 'show_admin', get_bloginfo('template_url') . '/library/styles/show_admin.css' );
	add_meta_box("gig-info", __("Show Information",'garageband'), "meta_options", "show", "normal", "low");
...
function meta_options(){
	...
	if( !preg_match( "/post-new/",$_SERVER['REQUEST_URI'],$matches ) && isset( $post->ID ) ):	
		...
		$admission = get_post_meta($post->ID,'admission',true);
	endif;
